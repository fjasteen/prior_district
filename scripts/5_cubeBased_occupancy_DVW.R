...
for (i in unique_districts) {
  # Read the status data for the current district
  status_data <- read_csv(paste0("./data/output/absent_sporadic_present/", i, "/absent-sporadic-present.csv"))
  colnames(status_data)[1] <- "species"  # Rename the first unnamed column to 'species'
  
  sub_data <- occupancy_districts %>% 
    filter(District == i) %>% 
    arrange(desc(`aantal km² in district`)) %>% 
    left_join(status_data, by = "species") %>% # Join with status data
    replace_na(list(status = "Absent")) %>%  # Join with status data
    mutate(status = as.factor(status))
  
  temp_plot <- ggplot(sub_data, 
                      aes(x = reorder(species, -`aantal km² in district`), 
                          y = `aantal km² in district`)) +
    geom_bar(aes(fill=status), stat = "identity") +
    scale_fill_manual(values = c("Present" = "red4", "Sporadic" = "orange3", "Absent" = "grey")) +
    coord_flip() +
    ggtitle(paste("Soorten in", i)) +
    xlab("Soort") +
    ylab("Aantal km² in district")
  ggsave(temp_plot, 
         file=paste0("./data/output/occupancy_DVW/Per_District/",clean(i),".png"), 
